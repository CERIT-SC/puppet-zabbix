Puppet::Type.newtype(:zabbix_host) do
  ...
  newproperty(:inventory_mode) do
    desc 'Set inventory mode: disabled(-1), manual(0), automatic(1).'
  end

  newproperty(:webservices, array_matching: :all) do
      desc 'Add webservices to the host'
    def insync?(is)
      is.sort_by{|entry| entry['name']} == should.sort_by{|entry| entry['name']}
    end
  end
